...
void getsize(Node* root,int &i){
    if(root==NULL){
        return;
    }
    getsize(root->left,i);
    i++;
    getsize(root->right,i);
}

int kth_largestnode(Node* root,int k){
    if(root==NULL){
        return -1;
    }
    int ans=-1;
...
int main(){
    ...
    int result=kth_largestnode(root,3);
    cout<<"Kth largest node of the tree is: "<<result<<endl;
